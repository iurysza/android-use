 ...
 */
export function isOk<T>(result: CommandResult<T>): result is CommandResult<T> & { success: true; data: T } {
 ...
 */
export function isErr<T>(result: CommandResult<T>): result is CommandResult<T> & { success: false; error: ResultError } {
  return !result.success;
}
 ...
 * Map success data
 */
export function map<T, U>(result: CommandResult<T>, fn: (data: T) => U): CommandResult<U> {
  if (isOk(result)) {
    const opts: {
      message?: string | undefined;
      warnings?: string[] | undefined;
